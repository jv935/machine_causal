...
class ExtendedSummaryGraph:
    ...
    2: arrow head ->
    """
    def __init__(self, nodes, order_matrix=pd.DataFrame()):
        super(ExtendedSummaryGraph, self).__init__()
        ...
        for node_present in self.nodes_present:
            ...
            for node_present_2 in self.nodes_present:
                if node_present != node_present_2:
                    present_node1_idx = self.nodes_present.index(node_present)
                    present_node2_idx = self.nodes_present.index(node_present_2)
                    node1 = nodes[present_node1_idx]
                    node2 = nodes[present_node2_idx]
                    if (node1 in order_matrix.columns) and (node2 in order_matrix.columns):
                        if order_matrix[node1].loc[node2] == 2:
...
def gtce(x, y, z=None, sampling_rate_tuple=(1, 1, 1), p_value=True, k=10, linear=True):
    '''
    ...
    if linear:
        ...
        cd = GPDC(significance='analytic')
    # if measure == "cmiknn":
    #     # cd = CMIknn(mask_type=None, significance='shuffle_test', sig_samples=1000,
    #     #             sig_blocklength=None, knn=k, confidence=None, conf_lev=0.9, conf_samples=100,
    #     #             conf_blocklength=None, verbosity=0) # , transform='rank'
    #     cd = CMIknn(mask_type=None, significance='shuffle_test', fixed_thres=None, sig_samples=10000,
    #                 sig_blocklength=3, knn=k, confidence=None, conf_lev=0.9, conf_samples=100,
    #                 conf_blocklength=1, verbosity=0) # , transform='rank'
    # else:
    #     cd = None
...
class CITCE:
    def __init__(self, series, sig_lev=0.05, lag_max=5, p_value=True, rank_using_p_value=False, verbose=True,
                 num_processor=-1, linear=True, order=pd.DataFrame()):
        """
        ...
        #     self.series[col] = self.series[col] + 0.001 * np.random.randn()
        # print(self.series)
        self.n = series.shape[0]
        self.d = series.shape[1]
        self.names = self.series.columns
        self.linear =linear
        self.graph = ExtendedSummaryGraph(self.names, order_matrix=order)
        ...
        for name_p in self.names:
            ...
            # print("############  Variance  ##########")
            # print(np.var(self.data_dict[node_p_past]), np.var(self.data_dict[node_p_present]))
            # print("######################")
    ...
    def window_representation_past_present(self, name_p, node_p_past, node_p_present, overlap=True, reduce=True):
        ...
        if self.lag_max == 0:
            ...
            if reduce:
                # ts_window_past = reduce_dim(ts_window_past, latent_dims=1)
                n_components = 1
                pca = PCA(n_components=n_components)
                ts_window_past = pd.DataFrame(pca.fit_transform(ts_window_past), columns=ts_window_past.columns[-n_components:])
    ...
    def _mi_pq(self, node_p, node_q):
        ...
        """
        x = self.data_dict[node_p] + 0.05 * np.random.randn()
        y = self.data_dict[node_q] + 0.05 * np.random.randn()
        mi_pval, mi_val = gtce(x, y, z=None, sampling_rate_tuple=(self.sampling_rate[node_p],
                                                                  self.sampling_rate[node_q]), p_value=self.p_value,
                               linear=self.linear)
        ...
        for pq in range(len(res)):
            ...
            if test:
                ...
                if (node_p in self.graph.nodes_present) and (node_q in self.graph.nodes_present):
                    if (node_q, node_p) in self.graph.ghat.edges:
    ...
    def _cmi_sep_set_pq(self, node_p, node_q, set_size):
        ...
        for set_nodes_r in nodes_r:
            ...
            cmi_pval, cmi_val = gtce(x, y, z, self.sampling_rate, p_value=self.rank_using_p_value, linear=self.linear)
            ...
            for node_p, node_q, r_set, cmi in zip(ranks.elem_p, ranks.elem_q, ranks.elem_r, ranks.val):
                ...
                if test:
                    ...
                    if self.p_value != self.rank_using_p_value:
                        ...
                        for node_r in r_set:
                            z[node_r] = self.data_dict[node_r]
                        cmi, _ = gtce(x, y, z, self.sampling_rate, p_value=self.p_value, linear=self.linear)
                    if self.verbose:
...
class PCGCE(CITCE):
    def __init__(self, series, sig_lev=0.05, lag_max=5, p_value=True, rank_using_p_value=False, verbose=True, num_processor=-1, linear=True):
        """
        ...
        :param num_processor: number of processors for parallelization. By default -1 (all)
        """
        CITCE.__init__(self, series, sig_lev, lag_max, p_value, rank_using_p_value, verbose, num_processor, linear)

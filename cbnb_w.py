...
class CBNBw:
    ...
    def constraint_based(self):
        ...
        if self.linear:
            measure = ParCorr(significance='analytic')
        else:
            measure = GPDC(significance='analytic')
        pcmci = PCMCI(dataframe=dataframe, cond_ind_test=measure, verbosity=0)
        ...
        #             elif (output["graph"][i, j, t] == "o-o") or (output["graph"][i, j, t] == "x-x"):
    ...
    # def find_instantaneous_nodes_with_undirected_edges(self):
    #     instantaneous_nodes = []
    #
    #     for i in range(len(self.data.columns)):
    #         for j in range(len(self.data.columns)):
        ...
        if len(instantaneous_nodes)>1:
            for idx in cycle_groups.keys():
                ...
                if self.linear:
                    ...
                    causal_order = run_resit(sub_data)
                print(causal_order)
